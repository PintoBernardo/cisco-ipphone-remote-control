...
class Cisco8841Phone(CiscoBasePhone):
    ...
    def build_ui(self):
        ...
        for l, u in self.config.get('line_keys_left', {}).items():
            tk.Button(lk_l, text="●", width=3, bg="#2a2a2a", fg="#27ae60", relief="flat", activebackground="#333",
                      command=lambda x=u: self.press(x), pady=7).pack(pady=7)

        # Screen
        sf = tk.Frame(top_f, bg="#121212", bd=2, relief="sunken"); sf.grid(row=0, column=1, padx=5)
        ...
        lk_r = tk.Frame(top_f, bg="#121212"); lk_r.grid(row=0, column=2, padx=5)
        for l, u in self.config.get('line_keys_right', {}).items():
            tk.Button(lk_r, text="●", width=3, bg="#2a2a2a", fg="#27ae60", relief="flat", activebackground="#333",
                      command=lambda x=u: self.press(x), pady=7).pack(pady=7)
        ...
        cc_f.pack(fill="none")
        for l, u in self.config.get('telephony_keys', {}).items():
        ...
        # Lower Grid
        lower_f = tk.Frame(self.main_container, bg="#121212"); lower_f.pack(pady=10)
        
        # Apps
        af = tk.Frame(lower_f, bg="#121212"); af.grid(row=0, column=0, padx=20)
        excluded = ["BACK", "MESSAGES", "VOL_UP", "VOL_DOWN"]
        ...
        for i, (l, u) in enumerate(app_items):
            tk.Button(af, text=l, width=12, bg="#2c3e50", fg="white", font=("Segoe UI", 8, "bold"), 
                      ...
                      command=lambda x=u: self.press(x)).grid(row=i//2, column=i%2, padx=3, pady=3)

        # Navigation
        nf = tk.Frame(lower_f, bg="#121212"); nf.grid(row=0, column=1, padx=20)
        ...
        # Keypad
        kf = tk.Frame(lower_f, bg="#121212"); kf.grid(row=0, column=2, padx=20)
        ...
        # Volume and Voicemail Cluster
        vf = tk.Frame(lower_f, bg="#121212"); vf.grid(row=0, column=3, padx=10)
        tk.Button(vf, text="✉ VOICEMAIL", width=14, bg="#d35400", fg="white", font=("Segoe UI", 8, "bold"),
                  relief="flat", activebackground="#e67e22",
                  command=lambda: self.press(ak.get('MESSAGES'))).pack(pady=5)
